...
function _webpushrSetLocalStorage(e, t) {
    existing_items = JSON.parse(localStorage.getItem("_webpushr")), new_items = { ...existing_items,
        [e]: t
    }, localStorage.setItem("_webpushr", JSON.stringify(new_items))
}

function _webpushrGetLocalStorage(e) {
    return existing_items = JSON.parse(localStorage.getItem("_webpushr")), existing_items && void 0 !== existing_items[e] ? existing_items[e] : ""
}

function _webpushrRemoveLocalStorage(e) {
    existing_items = JSON.parse(localStorage.getItem("_webpushr")), delete existing_items[e], localStorage.setItem("_webpushr", JSON.stringify(existing_items))
}

function _webpushrBrowserSupport() {
    if (_wp_debug && console.log("Webpushr: check browser support"), localStorage.getItem("_webpushrPromptAction")) {
        let e = JSON.parse(localStorage.getItem("_webpushrPromptAction")),
            t = new Date;
        if ("Dismiss" == e.action && e.expiry > t.getTime()) return _webpushrSessionLogs("session_prev_dismissed"), WebPushr.notificationCenter = _webpushrGetLocalStorage("notification_center"), WebPushr.notificationCard = _webpushrGetLocalStorage("notification_card"), 1 != WebPushr.notificationCenter.enabled && null != WebPushr.notificationCard && 1 != WebPushr.notificationCard.enabled || ("default" == Notification.permission ? _webpushrGetPrompt("get_info").then(e => {
            _webpushrGetPromptDisplayTime(e), _webpushrShowEmailOptin()
    ...
    return -1 != window.location.href.indexOf("http://") || "http" == WebPushr.integrationType ? "http" : "serviceWorker" in navigator ? "PushManager" in window ? "showNotification" in ServiceWorkerRegistration.prototype ? "denied" === Notification.permission ? (console.warn("WEBPUSHR: Notifications are denied by the user"), _webpushrGetPrompt("bell").then(e => {
        _webpushrShowSubscriptionBell(e, "denied"), _webpushrShowEmailOptin()
...
function _wp_registerServiceWorker() {
    "none" != WebPushr.swPath ? navigator.serviceWorker.register(WebPushr.swPath, {
        ...
        _wp_debug && console.log("Service worker has been registered successfully!"), WebPushr.promptWrapper = document.getElementById("webpushr-prompt-wrapper"), _webpushrCheckPermission()
    }, e => {
        console.log("Failed to register primary service worker, trying to register with alternative service worker"), navigator.serviceWorker.register("/webpushr-sw.js", {
            ...
            _wp_debug && console.log("Alternative service worker has been registered successfully!"), WebPushr.promptWrapper = document.getElementById("webpushr-prompt-wrapper"), _webpushrCheckPermission()
        }, e => {
...
function _webpushrCheckPermission() {
    ...
    })) : void _webpushrGetPrompt("get_info").then(e => {
        _webpushrGetPromptDisplayTime(e)
    });
    "default" == Notification.permission ? (_webpushrGetPrompt("get_info").then(e => {
        _webpushrGetPromptDisplayTime(e)
    }), _webpushrSessionLogs("session_new_to_push")) : "granted" == Notification.permission && (_wp_debug && console.log("User has previously granted push permission"), WebPushr.welcomeNotification = 0, _webpushrSessionLogs("session_subscribed"), _wpCheckSubscription(), _webpushrGetPrompt("bell").then(e => {
        _webpushrShowSubscriptionBell(e, "granted"), _webpushrShowEmailOptin(), _webpushrGetCookie("_webpushrTopicInterval") || _webpushrShowTopicPreferencesOptin()
    }))
}
async function _webpushrGetPrompt(e) {
    ...
    return _wp_prompt_info = await t.json(), WebPushr.notificationCenter = _webpushrGetLocalStorage("notification_center"), _webpushrSetLocalStorage("notification_center", { ...WebPushr.notificationCenter,
        enabled: _wp_prompt_info.notification_center
    }), WebPushr.notificationCenter = { ...WebPushr.notificationCenter,
        enabled: _wp_prompt_info.notification_center
    }, WebPushr.notificationCard = _webpushrGetLocalStorage("notification_card"), _webpushrSetLocalStorage("notification_card", { ...WebPushr.notificationCard,
        enabled: _wp_prompt_info.notification_card.enabled
    }), WebPushr.notificationCard = { ...WebPushr.notificationCard,
        enabled: _wp_prompt_info.notification_card.enabled
    }, _wp_prompt_info
}
...
function _webpushrShowCard(e) {
    ...
    t.className = "show-webpushr-notification-card", 1 == _wp_prompt_info.notification_card.auto_hide && setTimeout(function() {
        t.className = ""
    }, 1e3 * _wp_prompt_info.notification_card.auto_hide_delay), WebPushr.notificationCard = _webpushrGetLocalStorage("notification_card"), _webpushrSetLocalStorage("notification_card", {
        enabled: "1",
        id: e
...
function _webpushrPermissionResetInstructions() {
    denied_image_ele = document.getElementById("webpushrDeniedImg"), denied_image_ele && (WebPushr.promptWrapper.className = "webpushr-subscribed webpushr-notification-denied")
}

function _webpushrPromptAction(e) {
    _wp_debug && console.log("Prompt action : " + e), sessionStorage.setItem("_webpushrPromptAction", e), _webpushrPromptImpressions(e), "function" == typeof webpushrPermissionAction && webpushrPermissionAction(e);
    var t = document.querySelectorAll(".webpushr-subscription-topic");
    if (document.getElementById("webpushr-lightbox-optin") && document.getElementById("webpushr-lightbox-optin").remove(), clearTimeout(WebPushr.promptDelay), "Dismiss" == e) {
        let t = new Date;
        expiry = t.getTime() + 24 * _wp_prompt_info.re_show_prompt * 60 * 60 * 1e3, localStorage.setItem("_webpushrPromptAction", JSON.stringify({
            action: e,
            expiry: expiry
        })), _webpushrShowEmailOptin()
    }
    if ("Approve" == e) {
        for (var o = 0; o < t.length; o++) t[o].checked && WebPushr.subscriptionTopics.push(t[o].value);
        _wp_prompt_info.domain_label ? window.open(_wp_prompt_info.domain_label + "/subscribe?public_key=" + applicationServerKey, "wpushr-subscribe-dialog", "width=626, height=436, resizable=0, scrollbars=0, status=0, titlebar=0, left=" + (screen.width - 626) / 2 + ", top=" + (screen.height - 436) / 2) : _webpushrRequestPermission()
    }
}

function _webpushrShowEmailOptin() {
    if (!document.getElementById("webpushr-email-optin") && !_webpushrGetCookie("_webpushrEmailAction") && _wp_prompt_info.email_optin) {
        WebPushr.promptWrapper.innerHTML += _wp_prompt_info.email_optin, _webpushrCustomPromptEvents(_wp_prompt_info);
        var e = document.querySelector("#webpushr-optin-phone");
        if (e) {
            var t = document.createElement("script");
            t.type = "text/javascript", t.src = "https://cdnjs.cloudflare.com/ajax/libs/intl-tel-input/17.0.12/js/intlTelInput.js", document.head.appendChild(t), setTimeout(() => {
                window.intlTelInput(e, {
                    separateDialCode: !0,
                    initialCountry: e.dataset.defaultCountry
                })
            }, 6e3)
        }
    }
...
function _webpushrEmailAction(e) {
    if (_wp_debug && console.log("Email optin action : " + e), "Approve" == e) {
        if (event.preventDefault(), email_input = document.getElementById("webpushr-optin-email"), email = email_input && email_input.value, email_input) {
            if (email_validation = !0, email && !/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email) && (email_validation = !1), email_input.dataset.required && !email && (email_validation = !1), 0 == email_validation) return document.getElementById("webpushr-prompt-error-text").style.display = "block", email_input.style.borderColor = "red", email_input.focus(), email_input.select(), !1;
            document.getElementById("webpushr-prompt-error-text").style.display = "none", email_input.style.borderColor = "#ced3d8"
        }
        if (phone_input = document.getElementById("webpushr-optin-phone"), phone = phone_input && phone_input.value, phone_input) {
            if (phone_validation = !0, phone && !/^\d+$/.test(phone) && (phone_validation = !1), phone_input && phone_input.dataset.required && !phone && (phone_validation = !1), 0 == phone_validation) return document.getElementById("prompt-phone-error-text").style.display = "block", phone_input.style.borderColor = "red", phone_input.focus(), phone_input.select(), !1;
            document.getElementById("prompt-phone-error-text").style.display = "none", phone_input.style.borderColor = "#ced3d8", phone = document.querySelector(".webpushr-collect-phone .iti__selected-dial-code").innerText + phone
        }
        if (!email && !phone) return void(document.getElementById("webpushr-email-optin") && document.getElementById("webpushr-email-optin").remove());
        email && _webpushrSetLocalStorage("email", email), phone && _webpushrSetLocalStorage("phone", phone), _webpushrSendEmailToServer(email, phone)
    } else _webpushrSetCookie("_webpushrEmailAction", e, _wp_prompt_info.email_optin_reshow), document.getElementById("webpushr-email-optin") && document.getElementById("webpushr-email-optin").remove()
}

function _webpushrSendEmailToServer(e, t) {
    _wp_debug && console.log("send email form to server"), document.getElementById("webpushr-email-optin-form").style.display = "none", document.getElementById("webpushr-email-form-status").style.display = "flex", _webpushrSetCookie("_webpushrEmailAction", "Submitted", 90);
    var o = {
        endpoint: _webpushrGetLocalStorage("endpoint"),
        site_id: applicationServerKey,
        email: e,
        phone: t,
        timezone: (new Date).getTimezoneOffset()
    };
    fetch("https://bot.webpushr.com/subscribe/email", {
        body: JSON.stringify(o),
        method: "POST"
    }).then(e => e.text()).then(function(e) {
        document.getElementsByTagName("webpushrPromptConatiner")[0].className += " Slide-in-reverse", setTimeout(() => document.getElementById("webpushr-email-optin").remove(), 7e3)
    })
}

function _webpushrAssignTopicsAsAttributes() {}
...
function _webpushrToggleEditNotificationPopup() {
    ...
    }), WebPushr.notificationCenter = _webpushrGetLocalStorage("notification_center"), _webpushrSetLocalStorage("notification_center", { ...WebPushr.notificationCenter,
    ...
    Notification.requestPermission().then(e => {
        switch (e) {
            ...
            case "denied":
                ...
                t && (t.disabled = !0, t.checked = !1), _webpushrSendSubscriptionToServer("", "DELETE"), _webpushrShowEmailOptin();
                break;
            case "granted":
                "function" == typeof webpushrPermissionAction && webpushrPermissionAction("granted"), WebPushr.subscriptionButton && (WebPushr.subscriptionButton.style.display = "none", document.getElementById("webpushr-subscriber-count") && (document.getElementById("webpushr-subscriber-count").style.display = "none")), _webpushrGetPrompt("bell").then(e => {
                    _webpushrShowSubscriptionBell(e, "granted")
                }), _webpushrRemoveLocalStorage("bell_notification"), localStorage.removeItem("_webpushrPromptAction"), _webpushrSubscribeNow()
        }
        WebPushr.silentSubscribed || _webpushrPromptImpressions(e)
    }), 1 == _wp_prompt_info.prompt_type && _webpushrShowEmailOptin()
}
...
function _webpushrSendSubscriptionToServer(e, t, o = "") {
    if (e && !_wp_is_safari) {
        ...
        var i = {
            ...
            timezone: (new Date).getTimezoneOffset(),
            email: _webpushrGetLocalStorage("email"),
            phone: _webpushrGetLocalStorage("phone")
        }
    ...
    fetch("https://bot.webpushr.com/subscribe/" + o, {
        ...
        WebPushr.execute_hooks = !0, _webpushrExecuteHooks(), _webpushrSetLocalStorage("topics", WebPushr.subscriptionTopics), void 0 !== _wp_prompt_info.re_show_preferences && _webpushrSetCookie("_webpushrTopicInterval", 1, _wp_prompt_info.re_show_preferences), $attributes = {}, Object.keys(WebPushr.subscriptionTopics).length > 0 ? (WebPushr.subscriptionTopics.forEach(function(e, t, o) {
            topic_attr = "Topic: " + e, $attributes = { ...$attributes,
                [topic_attr]: "true"
            }
        }), publicMethods.attributes($attributes)) : WebPushr.welcomeNotification || "POST" != i.type || _webpushrShowTopicPreferencesOptin(), "undefined" != typeof _webpushrScriptReady && "function" == typeof _webpushrScriptReady && _webpushrScriptReady(), _webpushrShowEmailOptin()
    })
}
...
function _webpushrNotificationCardLogs(e) {
    (document.getElementsByClassName("promptPopup-show")[0] || "bell_opened" != e) && (data = {
        status: e,
        site_id: applicationServerKey
...
function _webpushrCustomPromptEvents(e) {
    ...
    }), eleUpdateTopicsBtn = document.getElementById("webpushr-update-topics-preferences"), eleUpdateTopicsBtn && eleUpdateTopicsBtn.addEventListener("click", function() {
        _webpushrUpdateTopicsPreferences()
...
function webpushr_display_toggle_button() {
    ...
    let o = "";
    "default" == Notification.permission ? o = "" : "denied" == Notification.permission ? o = " disabled " : "granted" == Notification.permission && (o = "off" == _webpushrGetLocalStorage("bell_notification") ? "" : " checked "), e.innerHTML = "<input " + o + ' id="webpushrSubscriptionToggleButton" type="checkbox"><label for="webpushrSubscriptionToggleButton"></label>';
    ...
    if (i.style.transform = "scale(" + e.dataset.size + ")", e.style.width = i.getBoundingClientRect().width + "px", e.style.height = i.getBoundingClientRect().height + "px", "default" == Notification.permission) n.addEventListener("click", _webpushrRequestPermission);
...
function _webpushrUpdateTopicsPreferences() {
    var e = document.querySelectorAll(".webpushr-subscription-topic");
    WebPushr.subscriptionTopics = [];
    for (var t = 0; t < e.length; t++) e[t].checked && WebPushr.subscriptionTopics.push(e[t].value);
    _webpushrSetLocalStorage("topics", WebPushr.subscriptionTopics), $attributes = {}, Object.keys(WebPushr.subscriptionTopics).length > 0 ? (WebPushr.subscriptionTopics.forEach(function(e, t, o) {
        topic_attr = "Topic: " + e, $attributes = { ...$attributes,
            [topic_attr]: "true"
        }
    }), publicMethods.attributes($attributes)) : publicMethods.attributes({
        "Topic: ": "None"
    }), _webpushrSetCookie("_webpushrTopicInterval", 1, _wp_prompt_info.re_show_preferences), document.getElementById("webpushr-lightbox-optin").remove()
}

function _webpushrShowTopicPreferencesOptin(e = 5e3) {
    _wp_prompt_info.topics && setTimeout(function() {
        WebPushr.promptWrapper.innerHTML = _wp_prompt_info.topics;
        var e = document.querySelectorAll(".webpushr-subscription-topic");
        if (selected_topics = _webpushrGetLocalStorage("topics"), selected_topics.length > 0)
            for (var t = 0; t < e.length; t++) - 1 === selected_topics.indexOf(e[t].value) && e[t].removeAttribute("checked");
        else
            for (t = 0; t < e.length; ++t) e[t].checked = !1;
        document.getElementById("_prompt_overlay").style.display = "block", _webpushrCustomPromptEvents(_wp_prompt_info)
    }, e)
}
...
var _wp_is_safari = !1,
    ...
    publicMethods = {
        ...
        init: function(e, t, o = "/", i = "https") {
            if (WebPushr.swScope = o, WebPushr.integrationType = "popup" == i ? "http" : "https", !WebPushr.swPath) switch (prompt_wrapper = document.createElement("div"), prompt_wrapper.setAttribute("id", "webpushr-prompt-wrapper"), document.body.appendChild(prompt_wrapper), WebPushr.promptWrapper = document.getElementById("webpushr-prompt-wrapper"), console.log("Web Push Notifications powered by Webpushr"), applicationServerKey = e, WebPushr.swPath = t || "/webpushr-sw.js", _webpushrBrowserSupport()) {
                case !0:
        ...
        event: function(e) {
            return _wp_debug && console.log("Run Webpushr Custom Event hook"), e.event_name ? e.event_action ? (data = { ...e,
                sid: WebPushr.endpoint,
                site_id: applicationServerKey
        ...
        attributes: function(e) {
            ...
            localStorage.getItem("_webpushrCustomAttribute") != JSON.stringify(e) && (_wp_debug && console.log("Attribute cookie not found. Send fetch request now"), localStorage.setItem("_webpushrCustomAttribute", JSON.stringify(e)), data = { ...e,
                sid: WebPushr.endpoint,
                site_id: applicationServerKey
        ...
        optin: function(e) {
            WebPushr.optin_text = e
        }
    };
window.webpushr = function() {
    publicMethods[arguments[0]].apply(this, Array.prototype.slice.call(arguments, 1))
}, _webpushrExecuteHooks("debug"), _webpushrExecuteHooks("setup"), _webpushrExecuteHooks("init"), _webpushrExecuteHooks("optin"), window.onmessage = function(e) {
    void 0 !== _wp_prompt_info && void 0 !== _wp_prompt_info.domain_label && _wp_prompt_info.domain_label.toLowerCase() == e.origin && (_webpushrSetCookie("_webpushrSubscriberID", e.data, 365), WebPushr.promptWrapper && WebPushr.promptWrapper.remove(), WebPushr.endpoint = e.data)
}, document.getElementById("webpushr-subscription-button") && "Safari" != WebPushr.userBrowser && webpushr_display_button(), document.getElementById("webpushr-subscription-toggle-button") && "Safari" != WebPushr.userBrowser && webpushr_display_toggle_button();